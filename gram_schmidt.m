function [p_coef, phi, alpha] = gram_schmidt(f, a, b, n, w)
    ...
    if n >= 1
        % Cálculo de B1
        num_B1 = integral(@(x) x .* w(x) .* polyval(phi{1}, x).^2, a, b);
        den_B1 = integral(@(x) w(x) .* polyval(phi{1}, x).^2, a, b);
        B(1) = num_B1 / den_B1;
    ...
    for k = 2:n
        ...
        num_Bk = integral(@(x) x .* w(x) .* polyval(phi{k}, x).^2, a, b);
        den_Bk = integral(@(x) w(x) .* polyval(phi{k}, x).^2, a, b);
        B(k) = num_Bk / den_Bk;

        num_Ck = integral(@(x) x .* w(x) .* polyval(phi{k}, x) .* polyval(phi{k-1}, x), a, b);
        den_Ck = integral(@(x) w(x) .* polyval(phi{k-1}, x).^2, a, b);
        C(k) = num_Ck / den_Ck;
    ...
    for k = 0:n
        num_alpha = integral(@(x) w(x) .* f(x) .* polyval(phi{k+1}, x), a, b);
        den_alpha = integral(@(x) w(x) .* polyval(phi{k+1}, x).^2, a, b);
